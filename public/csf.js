...
var DropSheet = function DropSheet(opts) {
		...
		worker.onmessage = function(e) {
			switch(e.data.t) {
				...
				case 'xls': case 'xlsx': pending = false;
					opts.on.workend();
					if(e.data.d) cb(JSON.parse(e.data.d), e.data.t);
					break;
			}
		};
	...
	function handleDropData(data) {
		...
		if(data.length > 500000) opts.errors.large(data.length, function(e) { if(e) doit(); });
		else { doit(); }
	}
	...
	if(opts.drop && opts.drop.addEventListener) {
	...
	function handleRawData(arr) {
		var data = arr.map(function(x){return String.fromCharCode(x);}).join("");
		xls = [0xd0, 0x3c].indexOf(data[0]) > -1;
		if(!xls && data.charCodeAt(0) !== 0x50)
			return opts.errors.badfile();
	...
	function handleData(d,raw) {return raw?handleRawData(d):handleDropData(d);}

	function out(d) { return pending ? opts.errors.pending() : handleData(d, 1); }
